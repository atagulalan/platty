...
function testUserListCounts(): void {
  ...
  list.upsert("Alice", {
    room: "room",
    ready: true,
    file: { name: "a.mkv", size: 1, duration: 100 },
  });
  list.upsert("Bob", {
    room: "room",
    ready: true,
  ...
  assert.strictEqual(
    list.usersInRoomCount("room", "Alice"),
    2,
    "counts self + ready users with files",
  );
  assert.strictEqual(
    list.readyUserCount("room", "Alice"),
    2,
    "counts ready self + ready others with files",
  );
  ...
  assert.strictEqual(
    list.onlyUserInRoomWhoSupportsReadiness("room", "Alice"),
    false,
    "Bob has a file",
  );
}
...
function testRequireSameFilenames(): void {
  ...
  list.upsert("Bob", {
    room: "room",
    ready: true,
    file: { name: "b.mkv", size: 1, duration: 100 },
  });
