      ...
      <Box flexDirection="column" height={height} overflow="hidden">
        {Array.from({ length: height }, (_, i) => {
          if (i < chatRows) {
            ...
            return <Text key={`empty-${start + i}`}>{fitLogLine("", contentWidth)}</Text>;
          }

          const overlayIndex = i - chatRows;
          const overlay = overlaySuggestions!;
          const line = renderSuggestionOverlayLine(overlay, overlayIndex, contentWidth);
          return (
            <Text key={line.key} dimColor={!line.selected} inverse={line.selected}>
              {line.text}
            </Text>
          );
        })}
