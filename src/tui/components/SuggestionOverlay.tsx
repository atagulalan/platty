...
export function suggestionScrollOffset(selectedIndex: number | null, totalCount: number): number {
...
export function renderSuggestionOverlayLine(
  ...
  if (overlayIndex === 0) {
    const scrollOffset = suggestionScrollOffset(overlay.selectedIndex, overlay.items.length);
    ...
    const hint = moreAbove && moreBelow ? " ▲▼" : moreAbove ? " ▲" : moreBelow ? " ▼" : "";
  ...
  const selected = overlay.selectedIndex !== null && itemIndex === overlay.selectedIndex;
