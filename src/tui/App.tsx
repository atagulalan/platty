...
function cyclePanel(current: PanelFocus, direction: 1 | -1): PanelFocus {
  ...
  if (idx === -1) {
    ...
    return direction === 1 ? PANEL_CYCLE[0]! : PANEL_CYCLE[PANEL_CYCLE.length - 1]!;
  }
  return PANEL_CYCLE[(idx + direction + PANEL_CYCLE.length) % PANEL_CYCLE.length]!;
}
...
const PANEL_HINT: Record<Exclude<PanelFocus, "input">, string> = {
  ...
  playlist: "panel mode: playlist — ↑/↓ move · enter select · d delete · esc input",
};
...
export function App({
  ...
  const logContentWidth = Math.max(1, (stdout.columns ?? 80) - LEFT_PANEL_WIDTH - 4);
  ...
  useEffect(() => {
    ...
    const onConnected = (info: {
      ...
      pushLineRef.current(`Connected as ${info.username} in room "${info.room}"`);
    ...
    const onUserlistUpdate = (): void => {
      ...
      if (debug)
        pushLineRef.current(`[debug] user list updated (${client.userList.all().length} users)`);
    };
    ...
    const onError = (message: string): void => pushLineRef.current(`Error: ${message}`, "error");
  ...
  useEffect(() => {
    setPlaylistCursor((c) =>
      playlist.files.length === 0 ? 0 : Math.max(0, Math.min(c, playlist.files.length - 1)),
    );
  ...
  useEffect(() => {
    setUsersCursor((c) => (users.length === 0 ? 0 : Math.max(0, Math.min(c, users.length - 1))));
  }, [users.length]);
  ...
  useInput((input, key) => {
    ...
    if (focusedPanel === "users") {
      ...
      if (key.upArrow) setUsersCursor((c) => (count === 0 ? 0 : Math.max(0, c - 1)));
      if (key.downArrow) setUsersCursor((c) => (count === 0 ? 0 : Math.min(count - 1, c + 1)));
      return;
    }
  });
    ...
    <Box flexDirection="column">
      <Box flexDirection="row" width="100%">
        ...
        <LogPanel
          ...
          overlaySuggestions={focusedPanel === "input" ? inputSuggestions : null}
        />
      </Box>
      <CommandInput
